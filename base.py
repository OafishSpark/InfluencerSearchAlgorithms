...
if __name__ == "__main__":
    ...
    # influencers = greedy_kkt(graph, sir_inf_model, 10)
    influencers = centrality_influencers(graph, 10)
    result_thrshld, result_sir = 0, 0
    n_experiments = 10
    for i in range(n_experiments):
        result_thrshld += (
            len(linear_threshold_model(graph, influencers, 5)) / n_experiments
        )
    ...
    print(
        f"Result of the infinite SIR model: {len(sir_inf_model(graph, influencers, 5))}"
    )
