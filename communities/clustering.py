...
def topological_distance(
    ...
    first, second = nodes
    k_1 = sum([graph.edges[first, elem]['weight'] 
               for elem in list(graph.adj[first])])
    k_2 = sum([graph.edges[second, elem]['weight'] 
               for elem in list(graph.adj[second])])
    k_12 = sum([
        graph.edges[first, elem]['weight'] + 
        graph.edges[second, elem]['weight'] 
